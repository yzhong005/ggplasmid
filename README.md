...
The figure immediately below is generated by the exact code shown here.
...
p_circular <- ggplasmid(
  ...
  palette = "npg",                            # ggsci palette name
  ...
  label_text_colour = "black",                 # fixed label text colour
...
### Plot a selected sequence region

Use `region_start` and `region_end` for a 1-based inclusive window. Features
are clipped and rebased, and the GenBank `ORIGIN` sequence is sliced before GC
tracks are calculated. With annotation-table input, the supplied FASTA record
is sliced to the same window:
...
Linear maps also support placing labels on both sides of each genome line with
`linear_label_side = "both"`. See the [linear-map wiki page](https://github.com/yzhong005/ggplasmidZY/wiki/Linear-map)
for the complete code and a rendered example.
...
```r
ggplasmid(
  gbk = "plasmid.gbk",             # ORIGIN is used for GC tracks
  name = "pExample",
  layout = "circular",
...
`category` controls gene-arrow classification and the selected `palette`; it is
not interpreted as a label colour column. `gene_highlight()` changes gene-arrow
fill colours only. For label text and leader lines, use columns containing
valid R colours:
